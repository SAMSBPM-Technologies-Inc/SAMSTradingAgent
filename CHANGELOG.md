...
**One drawing of what the system does**, on the landing page, in the README and
in the architecture doc. Four stages — ingest, score, confirm, act — with the
deep-research fan-out beneath them carrying the constraint that matters written
on it (*may veto a buy · never creates one, never reaches an exit*), and every
outcome looping back as win rate, calibration and profit net of commission. It
is generated by `scripts/render_pipeline_diagram.py` rather than drawn twice:
geometry once, both palettes derived, so the light and dark versions cannot
drift and the docs cannot end up illustrating a different product from the
website. On a phone it scrolls inside its own container — scaled to fit, the
labels land at about 3px — and the four steps in words below say the same thing
for a reader who does not pan.

**The sign-in page had no ground under it.** A form floating in an empty field.
It now sits on the same backdrop language as the landing page: one warm wash off
the top-left, a cooler counterweight bottom-right, and a fine rule grid that
fades out before it reaches the card. Drawn in CSS rather than photographed — a
photograph would need a second asset for dark mode, would be the heaviest thing
on the page, and would compete with the one control the screen exists for.

### Known gaps

- The landing page now states the four steps twice, once in the diagram and
  once in words. That is deliberate for the phone case, but it is repetition,
  and if the page is ever cut for length this is the first thing to look at.
- The diagram is a static asset regenerated by hand. Nothing fails if the
  pipeline changes and the script is not re-run — no test asserts the drawing
  still matches the code it describes.
- The numbers in it are illustrative, drawn from the same sample as the hero
  readout rather than from a live ticker.
